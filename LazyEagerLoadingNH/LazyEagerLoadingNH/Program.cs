    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                using (ISession session = sessionFactory.OpenSession())
                {
                    IList<Book> books = session.QueryOver<Book>().List();// CreateCriteria<Book>().List<Book>();
                ...
                using (ISession session = sessionFactory.OpenSession())
                {
                    IList<Author> authors = session.QueryOver<Author>().List();// CreateCriteria<Author>().List<Author>();
                ...
                using (ISession session = sessionFactory.OpenSession())
                {
                    IList<Book> books = session.QueryOver<Book>().Fetch(b => b.Author).Eager.List();// CreateCriteria<Book>().SetFetchMode("Author", FetchMode.Eager).List<Book>();
                ...
                using (ISession session = sessionFactory.OpenSession())
                {
                    IList<Author> authors = session.QueryOver<Author>().Fetch(a => a.Books).Eager.List();// CreateCriteria<Author>().SetFetchMode("Books", FetchMode.Eager).List<Author>();
                    foreach (Author a in authors)
                    {
                        Console.Out.WriteLine(a.Name + " books:");
                        foreach (Book b in a.Books)
